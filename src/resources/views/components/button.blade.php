...
@php
    $base =
        'inline-flex items-center py-1.5 px-4 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none';
    $variantBase = 'rounded-lg border focus:ring-1';
    ...
    $variantClasses = match ($variant) {
        ...
        'link' => 'text-gray-700 hover:text-gray-900 hover:underline',

        'none' => '',

        default => throw new InvalidArgumentException("Unknown button variant: {$variant}"),
    };

    $classes = $base . ' ' . ($variant !== 'link' && $variant !== 'none' ? $variantBase : '') . ' ' . $variantClasses;
@endphp

<button {{ $attributes->merge(['type' => 'button'])->class([$classes, 'gap-1' => $icon]) }}>
    @if ($icon)
